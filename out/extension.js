...
function createGroup(context, name) {
    const groups = loadGroups(context);
...
async function activateGroup(context, groupName) {
    const groups = loadGroups(context);
    const group = groups.find(g => g.name === groupName);
    if (group) {
        const disabledExtensions = await getDisabledExtensions();
...
async function deactivateGroup(context, groupName) {
    const groups = loadGroups(context);
    const group = groups.find(g => g.name === groupName);
    if (group) {
        const disabledExtensions = await getDisabledExtensions();
...
function getWebviewContent(extensions, groups) {
    ...
    const groupOptions = groups.map(group => `<option value="${group.name}">${group.name} (${group.extensions.length} extensions)</option>`).join('');
    return `
<!DOCTYPE html>
<html lang="en">
<head>
