                    ...
                    case "deassignGroup": {
                        if (message.name &&
                            ...
                            if (group) {
                                ...
                                const removedCount = initialLength -
...
function backupGroups() {
    try {
        ...
        fs.writeFileSync(backupPath, JSON.stringify({ groups }, null, 2), "utf8");
...
function collectExtensions(panel, context) {
    const defaultIcon = panel.webview
        .asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "media", "default-icon.png"))
        .toString();
...
// --------------------------------------------------------
async function offerCommandActions(title, commands) {
    const choice = await vscode.window.showInformationMessage(title, "Copy to Clipboard", "Show Commands", "Cancel");
    if (choice === "Copy to Clipboard") {
        ...
        vscode.window.showInformationMessage("Commands:", { modal: true, detail: commands });
    }
}
// --------------------------------------------------------
...
async function toggleExtension(extensionId) {
    ...
    await offerCommandActions(`${ext.isActive ? "Disable" : "Enable"} Extension`, cmd);
}
async function activateGroup(groupName) {
    ...
    const commands = group.extensions
        ...
        .join("\n");
    await offerCommandActions(`Enable all extensions in group "${groupName}"`, commands);
}
async function deactivateGroup(groupName) {
    ...
    const commands = group.extensions
        ...
        .join("\n");
    await offerCommandActions(`Disable all extensions in group "${groupName}"`, commands);
}
function deactivate() { }
