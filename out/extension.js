...
function collectExtensions(panel, context) {
    ...
    return vscode.extensions.all.map(ext => {
        ...
        return {
            id: ext.id,
            displayName: (pkg && pkg.displayName) || ext.id,
            description:
                (pkg && pkg.description) || "No description available",
            icon: iconUri,
            active: !!ext.isActive,
...
async function toggleExtension(extensionId) {
    try {
        ...
        vscode.window.showErrorMessage(
            `Failed to toggle extension: ${error}`
        );
    }
}
...
async function activateGroup(groupName) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(
            `Failed to activate group: ${error}`
        );
    }
}
...
async function deactivateGroup(groupName) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(
            `Failed to deactivate group: ${error}`
        );
    }
}
